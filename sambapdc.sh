...
if grep -qs "ubuntu" /etc/os-release; then
	...
	os_version=$(grep 'version_id' /etc/os-release | cut -d '"' -f 2 | tr -d '.')
        apt-get update
        apt-get upgrade -y
	apt-get install acl attr samba samba-dsdb-modules samba-vfs-modules winbind libpam-winbind libnss-winbind libpam-krb5 krb5-config krb5-user ntp smbclient -y
...
samba-tool domain provision --server-role=dc --use-rfc2307 --dns-backend=SAMBA_INTERNAL --realm=samba.example.com --domain=samba --adminpass=K231010Hkd$
